...
class BigDigitalView extends WatchUi.WatchFace {
    ...
    function onLayout(dc) {
        ...
        secondsLabel = View.findDrawableById("SecondsLabel");
        secondsLabelOrigX = secondsLabel.locX;
    }

    //! The user has just looked at their watch. Timers and animations may be started here.
    function onExitSleep() {
        isWithinGesture = true;
        if ($.gConfDisplaySeconds) {
            displaySeconds = true;
        }
    }
    ...
    function onUpdate(dc) {
        // clear the clip, if we're on a 2.3.x device (which supports 1Hz updates)
        if (Toybox.Graphics has :BufferedBitmap) {
            dcCurClip = null;
            dc.clearClip();
        }

        var useZenMode = App.getApp().getBooleanProperty("UseZenMode",true);
        if (useZenMode && !isWithinGesture) {
        ...
        if (displaySeconds || ($.gConfDisplaySeconds && Toybox.Graphics has :BufferedBitmap)) {
            updateSeconds(dc, localTimeInfo);
        } else {
        ...
        if ($.gDeviceSettings.activityTrackingOn) {
            ...
            if ($.gConfDisplayMoveIndicator) {
                updateMoveWarning(dc);
            }
        }
        if ($.gConfDisplayBatteryIndicator) {
            updateBatteryLevel(dc);
        }
    }

    function onPartialUpdate(dc) {
        // only need this if the seconds are configured to display
        if ($.gConfDisplaySeconds) {
            if (dcCurClip == null) {
                var tempLabel = View.findDrawableById("SecondsLabel");
                var textDims = dc.getTextDimensions("00",Gfx.FONT_MEDIUM);
                dcCurClip = [tempLabel.locX,tempLabel.locY,textDims[0],textDims[1]];
            }
    ...
    function updateSeconds(dc, localTimeInfo) {
        if ($.gConfDisplayMoveIndicator &&
...
class BigDigitalDelegate extends WatchUi.WatchFaceDelegate {
